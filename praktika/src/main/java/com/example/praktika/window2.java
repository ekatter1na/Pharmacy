...
public class window2 {
    ...
    void AnalizClikc(ActionEvent event) {
        int first = Integer.parseInt(aprel_col.getText()); //записываем в int цену
        int [] analiz  = {Integer.parseInt(aprel_col.getText()), //записываем в массив цены во всех аптеках
        Integer.parseInt(vita_col.getText()),
        Integer.parseInt(colHeath.getText()),
        Integer.parseInt(col24.getText())};
        for (int i = 0; i <= 3; i++){
            if(first > analiz[i]){  //перебираем и сравниваем каждый элемент массива
                first = analiz[i];
                i = 0;
            }
        }
